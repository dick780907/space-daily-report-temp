...
function renderDailyTab(container) {
  container.innerHTML = `
    <div class="space-y-4">
      ...
      <div class="bg-white rounded-2xl shadow-lg shadow-gray-100/50 border border-gray-100 overflow-hidden">
        <div class="overflow-x-auto" id="daily-table-wrapper">
          <!-- 動態生成 -->
        </div>
      </div>
    </div>
  ...
  // 日期變更時重新渲染
  dateInput.addEventListener('change', () => refreshDailyTable(dateInput.value));
  ...
  document.getElementById('daily-export-btn').addEventListener('click', () => {
    ...
    const csvRows = rows.map(r => ({
      ...
      '填表人': r.author || '',
      '辦公室查詢': r.officeQuery,
      '營業登記查詢': r.registerQuery,
      '參觀': r.visits,
      '辦公室續約': r.officeRenew,
      '辦公室新簽': r.officeNew,
      '營業登記新簽': r.registerNew,
      '營業登記續約': r.registerRenew,
      '簽約合計': r.signTotal,
      '退租': r.officeCancel,
      '付定': r.officeDeposit,
      '出租率': r.occupancyRate
    ...
    csvRows.push({
      ...
      '狀態': '',
      '填表人': '',
      '辦公室查詢': totals.officeQuery,
      '營業登記查詢': totals.registerQuery,
      '參觀': totals.visits,
      '辦公室續約': totals.officeRenew,
      '辦公室新簽': totals.officeNew,
      '營業登記新簽': totals.registerNew,
      '營業登記續約': totals.registerRenew,
      '簽約合計': totals.signTotal,
      '退租': totals.officeCancel,
      '付定': totals.officeDeposit,
      ...
      <tbody>
        ${rows.map(r => `
          <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
            ...
            <td class="px-3 py-3 text-center whitespace-nowrap">
              <span class="font-semibold ${r.occupancyRate >= 80 ? 'text-green-600' : r.occupancyRate >= 50 ? 'text-yellow-600' : 'text-red-500'}">
                ${r.occupancyRate}%
              </span>
            </td>
          </tr>
        ...
        <tr class="bg-purple-50 font-semibold text-purple-900">
          <td class="px-3 py-3 whitespace-nowrap" colspan="2">全館合計</td>
          ...
          <td class="px-3 py-3 text-center whitespace-nowrap">
            <span class="font-bold">${totals.occupancyRate}%</span>
          </td>
        </tr>
      </tbody>
    </table>
...
function renderMonthlyTab(container) {
  container.innerHTML = `
    <div class="space-y-4">
      ...
      <!-- 統計卡片 -->
      <div id="monthly-stats" class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
        <!-- 動態生成 -->
      </div>

      <!-- 月報表格 -->
      <div class="bg-white rounded-2xl shadow-lg shadow-gray-100/50 border border-gray-100 overflow-hidden">
        <div class="overflow-x-auto" id="monthly-table-wrapper">
          <!-- 動態生成 -->
        </div>
      </div>
    </div>
  ...
  document.getElementById('monthly-export-btn').addEventListener('click', () => {
    ...
    csvRows.push({
      ...
      '退租': totals.officeCancel,
      '付定': totals.officeDeposit,
      '填報天數': totals.fillDays,
      '平均出租率': totals.avgOccupancy + '%'
    });
      ...
      <tbody>
        ${rows.map(r => `
          <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
            ...
            <td class="px-3 py-3 text-center whitespace-nowrap">
              <span class="font-semibold ${r.avgOccupancy >= 80 ? 'text-green-600' : r.avgOccupancy >= 50 ? 'text-yellow-600' : 'text-red-500'}">
                ${r.avgOccupancy}%
              </span>
            </td>
          </tr>
        ...
        <tr class="bg-purple-50 font-semibold text-purple-900">
          ...
          <td class="px-3 py-3 text-center whitespace-nowrap">
            <span class="font-bold">${totals.avgOccupancy}%</span>
          </td>
        </tr>
      </tbody>
    </table>
...
function renderYearlyTab(container) {
  ...
  container.innerHTML = `
    <div class="space-y-4">
      ...
      <div class="bg-white rounded-2xl p-5 shadow-lg shadow-gray-100/50 border border-gray-100">
        <div class="flex flex-col sm:flex-row gap-4">
          ...
          <div>
            ...
            <select
              id="yearly-branch"
              class="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all min-w-[160px]"
            >
              ${branchOptions}
            </select>
          </div>
      ...
      <!-- 館別標題 -->
      <div id="yearly-branch-title" class="text-center">
        <!-- 動態生成 -->
      </div>

      <!-- 年報表格 -->
      <div class="bg-white rounded-2xl shadow-lg shadow-gray-100/50 border border-gray-100 overflow-hidden">
        <div class="overflow-x-auto" id="yearly-table-wrapper">
          <!-- 動態生成 -->
        </div>
      </div>
    </div>
  ...
  document.getElementById('yearly-export-btn').addEventListener('click', () => {
    ...
    const csvRows = months.map(m => ({
      '月份': m.month,
      '辦公室查詢': m.officeQuery,
      '營業登記查詢': m.registerQuery,
      '參觀': m.visits,
      '辦公室續約': m.officeRenew,
      '辦公室': m.officeNew,
      '營業登記新約': m.registerNew,
      '營業登記續約': m.registerRenew,
      '簽約合計': m.signTotal,
      '退租': m.officeCancel,
      '付定': m.officeDeposit,
      '填報天數': m.reportDays,
      '平均出租率': m.avgOccupancy + '%'
    }));
    csvRows.push({
      '月份': '年度合計',
      '辦公室查詢': totals.officeQuery,
      ...
      '參觀': totals.visits,
      '辦公室續約': totals.officeRenew,
      '辦公室': totals.officeNew,
      '營業登記新約': totals.registerNew,
      '營業登記續約': totals.registerRenew,
      '簽約合計': totals.signTotal,
      '退租': totals.officeCancel,
      '付定': totals.officeDeposit,
      '填報天數': totals.reportDays,
      '平均出租率': totals.avgOccupancy + '%'
    });
      ...
      <tbody>
        ${months.map(m => `
          <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
            ...
            <td class="px-3 py-3 text-center whitespace-nowrap">
              <span class="font-semibold ${m.avgOccupancy >= 80 ? 'text-green-600' : m.avgOccupancy >= 50 ? 'text-yellow-600' : 'text-red-500'}">
                ${m.avgOccupancy}%
              </span>
            </td>
          </tr>
    ...
    <div class="space-y-4">
      ...
      <div class="bg-white rounded-2xl p-5 md:p-6 shadow-lg shadow-gray-100/50 border border-gray-100">
        <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
          📋 今日填報狀態
          <span class="text-sm font-normal text-gray-500">(${formatDate(todayStr())})</span>
        </h3>
          ...
          ${status.map(s => `
            <div class="flex items-center gap-3 p-3 rounded-xl border ${s.filled ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}">
              ...
              <div class="min-w-0">
                ...
                <div class="text-xs ${s.filled ? 'text-green-600' : 'text-red-500'}">
                  ${s.filled ? escapeHtml(s.author) : '未填報'}
                </div>
              </div>
            </div>
          `).join('')}
        ...
        <div class="mt-4 flex items-center gap-4 text-sm text-gray-600">
          <span class="flex items-center gap-1">
            ...
            已填報 ${status.filter(s => s.filled).length} 館
          </span>
          <span class="flex items-center gap-1">
            <span class="w-2 h-2 rounded-full bg-red-500"></span>
            未填報 ${status.filter(s => !s.filled).length} 館
          </span>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="bg-white rounded-2xl p-5 md:p-6 shadow-lg shadow-gray-100/50 border border-gray-100">
          ...
          <button
            ...
            class="bg-gradient-to-r from-purple-600 to-purple-500 text-white px-6 py-3 rounded-xl font-medium shadow-lg shadow-purple-200 hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center gap-2"
          >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="bg-white rounded-2xl p-5 md:p-6 shadow-lg shadow-gray-100/50 border border-gray-100">
          ...
          <div class="flex flex-col sm:flex-row gap-3">
            ...
            />
            <button
              id="restore-import-btn"
              class="bg-white text-purple-600 border-2 border-purple-200 px-5 py-3 rounded-xl font-medium hover:bg-purple-50 transition-all flex items-center gap-2 whitespace-nowrap justify-center"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <div class="bg-white rounded-2xl p-5 md:p-6 shadow-lg shadow-red-100/50 border-2 border-red-200">
        ...
        <button
          id="clear-all-btn"
          class="bg-red-500 text-white px-6 py-3 rounded-xl font-medium hover:bg-red-600 transition-all flex items-center gap-2"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
